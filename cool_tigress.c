 ...
 * heating(x_e, x_HI, x_H2, nH, T, Z, xi_CR, G_PE, G_H2, flag_fast)
 ...
 * thus and over-estimates the final abundance of e-. This is faster but less
 * accurate.
 *
...
//heating---------------------------------------------------------------------
//cosmic ray heating
static Real heatingCR(const Real x_e, const Real x_HI, const Real x_H2,  
...
Real cooling(const Real x_e, const Real x_HI, const Real x_H2,
  ...
  const Real log10T_cold = 4.0;
  const Real log10T_hot = 4.2;
  ...
  if (log10T > log10T_hot) {
    ...
    log10L = linearInterp_(log10T_cold, log10T_hot, 
